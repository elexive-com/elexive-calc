...
const FeatureIntroduction = ({ 
  ...
  return (
    <div className="feature-introduction mb-8 relative overflow-hidden rounded-xl shadow-lg">
      {/* Background with stronger primary color */}
      <div className="absolute inset-0 bg-[var(--elexive-primary)] opacity-8"></div>
      
      {/* Main background with stronger contrast */}
      <div className="absolute inset-0 bg-[#EEF2F6] opacity-95"></div>
      
      {/* Decorative accents */}
      <div className="absolute bottom-0 left-0 right-0 h-1 bg-[var(--elexive-primary)]"></div>
        ...
        <div>
          <div className="flex-1">
            ...
            <div className="relative pb-1 mb-3">
              <h3 className="font-bold text-[var(--elexive-primary)] text-xl">
                {title}
              </h3>
              <span className="absolute -bottom-1 left-0 w-24 h-1.5 bg-[var(--elexive-accent)]"></span>
            </div>
            
            {/* Description with better contrast */}
            {typeof description === 'string' ? (
              <p className="text-[var(--elexive-primary)] text-sm mb-3 leading-relaxed font-medium">
                {description}
              </p>
            ) : (
              description
            ...
            {additionalInfo && typeof additionalInfo === 'string' ? (
              <div className="mt-4 pt-3 bg-white bg-opacity-70 p-3 rounded-lg border-l-3 border-[var(--elexive-accent)]">
                <p className="text-[var(--elexive-primary)] text-sm font-medium">
                  {additionalInfo}
                </p>
              </div>
            ) : (
